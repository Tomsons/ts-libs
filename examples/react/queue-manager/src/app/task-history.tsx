...
function TaskHistoryItem({ taskId, progresses, onDelete }: Props) {
	...
	const progressPercentage =
		lastProgress.status === TaskStatus.COMPLETED || lastProgress.error ? 100 : lastProgress.progress;
	const canBeDeleted = lastProgress.status === TaskStatus.COMPLETED || lastProgress.status === TaskStatus.FAILED;

	return (
		<div className="bg-white p-4 rounded-lg shadow mb-4">
			<div className="flex justify-between items-center mb-2">
				<h3 className="text-sm font-medium text-gray-700 truncate" title={taskId}>
					{taskId}
				</h3>
