...
describe('QueueManager', () => {
  ...
  it('should retry failed tasks', async () => {
    ...
    const { task, execute } = createTestTask({
      id: 'task1',
      executionTime: 10,
      shouldFail: true,
    })
    ...
    // Check final state
    const finalCall =
      progressListener.mock.calls[progressListener.mock.calls.length - 1][0]
    expect(finalCall).toEqual(
      expect.objectContaining({ taskId: 'task1', status: TaskStatus.FAILED })
    )
  ...
  it('should wait for completion', async () => {
    ...
    queueManager.enqueue(
      createTestTask({ id: 'task1', executionTime: 20 }).task
    )
    queueManager.enqueue(
      createTestTask({ id: 'task2', executionTime: 30 }).task
    )
  ...
  it('should handle a task with a timeout', async () => {
    vi.useFakeTimers()
    const queueManager = new QueueManager({ concurrency: 1, maxRetries: 0 })
    const progressListener = vi.fn()
    queueManager.onProgress(progressListener)
    ...
    const task: Task<void> = {
      ...
      execute: () =>
        new Promise((resolve) => {
          setTimeout(() => {
            resolve()
          }, 100) // This will exceed the timeout
        }),
    }

    queueManager.enqueue(task)
    await vi.runAllTimersAsync()
    queueManager.waitForCompletion()
    await vi.runAllTimersAsync()
    expect(progressListener).toHaveBeenNthCalledWith(
      3,
      expect.objectContaining({
        taskId: 'timeout-task',
        status: TaskStatus.FAILED,
        error: expect.any(TaskTimeoutError),
      })
    )
    vi.useRealTimers()
  })
